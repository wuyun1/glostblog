...
(function($) {
	...
	function render(elements, fadein) { //渲染元素
		...
		callback = function(holderobj) {
			...
			holderobj.find('.img_holder').css({
				'width': i_w,
				'height': i_h
			});
			imgobj.css({
				'width': i_w * setting.scaleimg,
		...
		$(elements).each(function(i) {
			...
			if ($(this)[0].nodeName.toLowerCase() == 'img' || $(this).find(setting.img_selector).length > 0) { //本身是图片或含有图片
				...
				// if (src == '') {
				// 	src = '/assets/images/commpt.png';
				// 	if ($(this)[0].nodeName.toLowerCase() == 'img') {
				// 		$(this).attr('src', src);
				// 	} else {
				// 		$(this).find(setting.img_selector).attr('src', src);
				// 	}
	...
	function onScroll() { //滚动加载
		...
		waterfall._scrollTimer = setTimeout(function() {
			if (waterfall.$columns.length <= 0) return;
